...
def build_xml_tree__Grad(dt, node_id, node_pos, parent_depth, parent_elementTree, scale):
    ...
    if (children_left[node_id] != children_right[node_id]):    
        ...
        node_elementTree = ET.SubElement(parent_elementTree, "Node", pos=pos, depth=depth, NCoef="0", IVar=IVar, 
            Cut=Cut, cType="1", res="0.0e+01", rms="0.0e+00", purity="0.0e+00", nType="0")
        build_xml_tree__Grad(dt, children_left[node_id], "l", node_depth, node_elementTree, scale)
        build_xml_tree__Grad(dt, children_right[node_id], "r", node_depth, node_elementTree, scale)
    else:
        # leaf node
        node_depth = parent_depth + 1
...
def convert_bdt__Grad(sklearn_bdt_clf, input_var_list, tmva_outfile_xml):
    ...
    for idx, dt in enumerate(clf.estimators_[:, 0]):
        ...
        build_xml_tree__Grad(dt, 0, "s", -1, BinaryTree, clf.learning_rate/2.)
